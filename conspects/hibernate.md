...
**Entity** объектное отражение данных в бд.\
требования к объекту **entity** в hibernate:
- должен иметь геттеры и сеттеры
- класс не может быть final и содержать final полей (потому что hibernate работает с прокси и изменяет эти объекты)
- должен иметь конструктор без параметров (потому что hibernate работает с рефлексией)
- тип **id** должен имплементировать интерфейс-маркер serializable
...
**Session** это интерфейс, содержит множество методов и занимается созданием sql запросов используя reflection api.\
**Session** однопоточный короткоживущий объект, предоставляет связь между объектами приложения и бд.\
**Session** по сути является оберткой над объектом **Connenction**.\
...
**@OneToMany** - (toMany ленивые) автор может иметь много книг. (mappedBy = "author") указывается колонка, по которой маппятся книги.\
**@ManyToOne** - (все toOne жадные) книга может иметь только одного автора.\
...
**@DynamicUpdate** - SQL включает только обновленные поля. тк накладывает доп расходы, использовать только в случае **optimistic lock** без @Version.\
...
![entity_state.png not found](img/entity_state.png)
...
- **Detached** - объект был persistent, но сейчас не связан с какой-либо сессией, то он находится в detached состоянии.\
из **persistent** в **detached** можно перейти используя операции с кешем или закрыв сессию **evict()**, **clear()**, **close()**\
из **detached** в **persistent** можно используя методы **update(), saveOrUpdate()**.
- **Removed** - при удалении сущности из сессии используя метод **delete()**.\
из **removed** переход сущности в любое другое состояние невозможен.\
...
**кеш 1 уровня** это **persistentContext** объекта **Session**.\
все сущности в рамках сессии сохраняются в ее кеш. Пока сессия не закрыта, можно изменять сущности в кеше,\
при коммите все изменения будут сохранены в бд. Tакая сессия называется **dirty session**.\
...
- **А** читает данные **Х**
- **В** читает данные **Х**
- **А** меняет данные **Х** на **ХА**
- **А** записывает данные **ХА**
- **В** меняет данные **Х** на **ХВ**
- **В** записывает данные **ХВ**
...
предотвращают сценарий, когда транзакции перезаписывают данные друг друга, не зная об этом.
- **оптимистичные** используются, когда перезапись данных **предполагается редко**.
- **пессимистичные** используются, когда перезапись данных **предполагается часто**.

**Оптимистичные блокировки:**\
`+` обеспечивают более конкурентный доступ к данным.\
`-` приходится повторять транзакции, не успевшие внести изменения раньше других.\
контролируются **JPA** и требует доп столбец в БД.\
**@Version** автоматом включает оптимистичную блокировку этой сущности. бывает **Numeric** и **TimeStamp**

**Пессимистичные блокировки:**\
`+` гарантируют отсутствие противоречий при выполнении транзакции.\
`-` блокирует данные сразу перед модификацией данных.\
`-` могут породить дедлоки, приневнимательном использовании. решение - использовать @Transactional timeout
блокировку нужно вызвать во всех транзакциях, тк мы не знаем какая будет выполнена первой.\
пессимистичекая блокировка использует механизмы БД.\
**@Lock** позволяет включить и настроить пессимистическую блокировку.

Блокировку также можно выставить через **EntityManager** вызвав метод **lock(obj, LOCK_MODE)**\
- **NONE** - нет блокировки
- **OPTIMISTIC** - сравнивает значение version, если отличается - бросается ошибка
- **OPTIMISTIC_FORCE_INCREMENT** - после commit значение version принудительно +1 чтобы если после commit мы еще хотим что-то сделать с\
данными, и не хотим, чтобы другие транзакции вклинились между 1 коммитом и закрытием транзакции
- **PESSIMISTIC_READ** - пессимистичная блокировка на чтение.
- **PESSIMISTIC_WRITE** - пессимистичная блокировка на запись и чтение
- **PESSIMISTIC_FORCE_INCREMENT** - пессимистичная блокировка на запись и чтение с принудительным увеличением поля Version.
